...
function queryList(){
    ...
    zhget("/rs/information_type",data).then(function (result) {
...
function onDeleteClick(id) {
    if(confirm("确定要删除吗？")) {
        zhput("/rs/information_type"+"/"+id,{status:9}).then(function(result){
...
function onSavecolumnData(){
    ...
    if(modelid==""||modelid==null||modelid==undefined){
        zhpost("/rs/information_type",{name:name}).then(function(result){
        ...
        zhput("/rs/information_type"+"/"+modelid,{name:name}).then(function(result){
