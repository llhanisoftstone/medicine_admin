...
function queryList(){
    ...
    zhget("/rs/event",data).then(function (result) {
...
function onDeleteClick(id) {
    if(confirm("确定要删除吗？")) {
        zhput("/rs/event"+"/"+id,{status:9}).then(function(result){
...
function onSavecolumnData(){
    ...
    if(modelid==""||modelid==null||modelid==undefined){
        zhpost("/rs/event",{name:name}).then(function(result){
        ...
        zhput("/rs/event"+"/"+modelid,{name:name}).then(function(result){
